        ...
        if message.text == "Привет" or message.text == "привет":
            ...
            if UserMale.male_female != 'male' and UserMale.male_female != 'female':
                bot.send_message(message.from_user.id,
                                 "Укажи кто ты: парень или девушка, чтобы найти себе пару с помощью стикеров")
            else:
                bot.send_message(message.from_user.id,
                                 text='Я начал искать тебе человека, когда найду обязательно напишу')
                user_parameters = [message.from_user.id, message.from_user.username, message.sticker.set_name,
                                   UserMale.male_female]
                ...
                for i in range(len(
                        data)):  # изменяем пол полльзователя во всех данных на пол который пользоавтель указал в текущем сеансе
                ...
                merge_matrix = merge_matrix.drop(['Unnamed: 0'],
                                                 axis=1)  # результатом операции drop ялвяются в том числе рваные номера
